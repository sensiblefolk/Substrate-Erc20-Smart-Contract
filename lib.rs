...
mod erc20 {
    ...
    impl Erc20 {
        ...
        pub fn transfer_from(&mut self, from: AccountId, to: AccountId, value: Balance) -> bool {
            ...
            let caller = self.env().caller();
            let spend_allowance = self.allowance_of_or_zero(&from, &caller);
            if spend_allowance < value {
                return false;
            };

            // Decrease the value of the allowance and transfer the tokens.
            self.allowances
                .insert((from, caller), spend_allowance - value);
        ...
        pub fn transfer(&mut self, to: AccountId, value: Balance) -> bool {
            ...
            self.transfer_from_to(sender, to, value)
        }

        fn transfer_from_to(&mut self, from: AccountId, to: AccountId, value: Balance) -> bool {
            let sender_balance = self.balance_of_or_zero(&from);
            let receiver_balance = self.balance_of_or_zero(&to);
